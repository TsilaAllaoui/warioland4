...
extern void func_800FD90(const u16*, u32, u32);
extern void func_800FF64(void);
extern void func_8010230(void);
extern void func_8012BAC(u8);
...
void SetBouncyWarioPose(u8 result)
{
    ...
    if (result == 0) {
        if (gWarioDataCopy.unk_1A == 2)
            result = 1;
        m4aSongNumStart(SE_BOUNCY_WARIO_HOP);
    } else {
...
void ProcessBouncyWarioCollision(void)
{
...

    {
        ...
        if (gUnk_3001918.unk_13 != 0 && gWarioData.pose == WPOSE_BOUNCY_JUMPING)
            result = 3;
    }
