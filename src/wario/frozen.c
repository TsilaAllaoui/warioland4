...
struct FrozenAnimationFrame { const u16 *oam; u32 unk4; u8 time; };
struct FrozenGraphicsFrame { const u8 *data; const u16 *oam; u32 unk8; };
...
extern s32 func_800FDBC(void);
extern s32 func_8014C4C(void);
extern s32 func_8014930(void);
extern s32 func_80143D8(void);
extern s32 func_8014758(void);
extern void func_800FF64(void);
extern void func_800FD90(const void *, u32, u32);

u8 UpdateFrozenWario(void) { return sFrozenWarioPoseTable[gWarioData.pose](); }
...
u8 FrozenWarioTransforming(void)
{
    ...
    if (wario->unk_1E >= animation[wario->unk_1F].time) {
        wario->unk_1E = 0;
        wario->unk_1F++;
        if (animation[wario->unk_1F].time == 0) return 1;
    }
    return 0xFF;
}
...
{
    if (gWarioData.horizontalDirection & 0x10) gWarioData.xVelocity = -96;
    else gWarioData.xVelocity = 96;
    return 0xFF;
}
...
void UpdateFrozenWarioMovement(void)
{
    ...
    wario = &gWarioData;
    index = wario->pose * 8; ptr = data + 1; index += (u32)ptr; hitbox[8] = *(u8 *)index;
    index = wario->pose * 8; ptr = data + 2; index += (u32)ptr; hitbox[9] = *(u8 *)index;
    index = wario->pose * 8; data += 3; index += (u32)data; hitbox[10] = *(u8 *)index;
    func_800FE58();
    movement = 0;
    if (wario->unk_1A == 2) {
...
void ProcessFrozenWarioCollision(void)
{
    ...
    data = sFrozenPoseData;
    wario = &gWarioData;
    index = wario->pose * 8; ptr = data + 4; index += (u32)ptr; hitbox[11] = *(u8 *)index;
    index = wario->pose * 8; ptr = data + 5; index += (u32)ptr; hitbox[12] = *(u8 *)index;
    index = wario->pose * 8; ptr = data + 6; index += (u32)ptr; hitbox[13] = *(u8 *)index;
    index = wario->pose * 8; data += 7; index += (u32)data; hitbox[17] = *(u8 *)index;
...
void UpdateFrozenWarioHitbox(void)
{
    ...
    hitboxes = (const u8 *)sUnk_82DEB18;
    offset <<= 3;
    address = offset + (u32)hitboxes;
    wario->hitboxOffsetLeft = *(u16 *)address;
    address = (u32)(hitboxes + 2); asm("" : "+r"(address)); address = offset + address; wario->hitboxOffsetTop = *(u16 *)address;
    address = (u32)(hitboxes + 4); asm("" : "+r"(address)); address = offset + address; wario->hitboxOffsetRight = *(u16 *)address;
    hitboxes += 6; offset += (u32)hitboxes; wario->hitboxOffsetBottom = *(u16 *)offset;
    poseOffset = wario->pose * 8; data += 7; poseOffset += (u32)data; value = *(u8 *)poseOffset;
    if (value == 2) wario->unk_1A = value;
    else if (wario->unk_1A != 1) wario->unk_1A = value;
}
