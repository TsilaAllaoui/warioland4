...
extern u8 gWarioHopSubState;
extern u8 gWarioHopNewHighScore;
extern u16 gWarioHopScore;
extern u16 gWarioHopMedalAnim[2];
extern u8 gWarioHopMedalIconActive;
extern struct WarioHopMidState gWarioHopSignState;
...
s32 func_8089B9C(u32 timer);
void func_807A428(void);
...
u32 WarioHopUpdateStateMachine(void)
{
    switch (gWarioHopSubState) {
        ...
        case 1:
            WarioHopScrollBackgrounds();
            WarioHopAdvancePlayerAnimation();
            gWarioHopSignState.angle -= 20;
            if (gWarioHopSignState.angle <= 31) {
                gWarioHopSignState.angle = 32;
                gWarioHopSignState.active = 0;
                gWarioHopSubState++;
            }
            break;
        case 2:
        ...
        case 3:
            ...
            if (func_8089B9C(120) != 0) {
                gWarioHopSubState++;
                gWarioHopSignState.active = 2;
            }
            break;
        case 4:
            WarioHopUpdateGameOverMusic();
            gWarioHopSignState.angle += 20;
            if (gWarioHopSignState.angle > 512) {
                m4aSongNumStart(0x236);
                gWarioHopSignState.angle = 512;
                gWarioHopSubState++;
            }
            break;
        case 5:
...
void WarioHopInit(void)
{
    ...
    *(vu16 *)0x04000054 = zero;
...

    {
        ...
        gWarioHopMedalAnim[0] = tailZero;
        gWarioHopMedalAnim[1] = tailZero;
        gWarioHopMedalIconActive = tailZero2;
    }

    gWarioHopSignState.xOffset = 0x80;
    gWarioHopSignState.yOffset = 0x40;
...
{
    if (gWarioHopMedalIconActive != 0) {
        func_807AB8C(gMedalCount, sUnk_8711D98, 0x3840);
    } else {
        func_807AB8C(gMedalCount, sUnk_8711AF8, 0x3840);
    }
    func_807AB8C(gWarioHopScore, sUnk_8711438, 0x38C0);
    func_8089C98(gMinigameHighScores[1], sUnk_8711438 + 0xC20, 0x4160);
}
...
s32 WarioHopAddScoreAndCheckMedal(void)
{
    ...
    if (gWarioHopScore > 999) {
        gWarioHopScore = 999;
    } else {
        m4aSongNumStart(0x224);
    }
    if (gWarioHopScore > gMinigameHighScores[1]) {
        gWarioHopNewHighScore = 1;
        gMinigameHighScores[1] = gWarioHopScore;
    }
    WarioHopDrawScoreDigits();
    if (gWarioHopScore % 15 == 0) {
        return 1;
    }
    return 0;
}
...
#define DRAW_FRAME_COMMON(_frame, _x, _y, _attr0Set, _attr0Clear, _attr1Set, _attr1Clear, _attr2Set, _attr2Clear) \
    ...
    effectActivePtr = &gWarioHopMedalIconActive;
    /* agbcc otherwise hoists this read across the preceding OAM loop. */
    asm("ldrb %0, [%1]" : "=r"(effectActive) : "r"(effectActivePtr));
    if (effectActive != 0) {
        gWarioHopMedalAnim[0]++;
        if (sUnk_870D428[gWarioHopMedalAnim[1]].time < gWarioHopMedalAnim[0]) {
            gWarioHopMedalAnim[0] = 1;
            gWarioHopMedalAnim[1]++;
            if (sUnk_870D428[gWarioHopMedalAnim[1]].time == 0) {
                register u32 resetZero asm("r0");
                register u8 *resetActivePtr asm("r2");

                gWarioHopMedalAnim[1] = 0;
                gWarioHopMedalAnim[0] = 0;
                resetZero = 0;
                resetActivePtr = &gWarioHopMedalIconActive;
    ...
    }
    {
        ...
        frame = sUnk_870D428[gWarioHopMedalAnim[1]].oam;
        count = *frame;
        total += count;
        frame++;
...
#undef DRAW_FRAME_COMMON
...

    {
        ...
        affineState = &gWarioHopSignState;
        affineScaleOffset = 4;
        asm("ldrsh %0, [%1, %2]" : "=r"(affineInverse) : "r"(affineState), "r"(affineScaleOffset));
        affineInverse = FixedInverse(affineInverse);
